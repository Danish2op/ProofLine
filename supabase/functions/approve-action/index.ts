Deno.serve(async (request) => {
  ...
  if (!isRecord(body) || !isRecord(body.buzzEvent)) {
    return error(
      'invalid_request',
      'A verified Buzz approval event is required.',
      400,
    );
  }
  if (
    ...
    return error(
      'invalid_request',
      'A complete verified Buzz approval payload is required.',
      400,
    );
  }
  const response = await rpc('apply_verified_buzz_approval', {
    ...
    source_raw_event_json: body.buzzEvent,
  });
  if (!response.ok)
    return error(
      'upstream_failure',
