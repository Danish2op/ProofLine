-- Task 7 fix: one server-only RPC records a verified observation and applies
-- its single allowed approval transition in the same database transaction.

drop function if exists public.apply_verified_buzz_approval(uuid, uuid, text, timestamptz, timestamptz, text, jsonb);
...
begin
  if jsonb_typeof(source_raw_event_json) <> 'object' then
    raise exception 'verified Buzz event must be an object';
  end if;
  ...
  if jsonb_typeof(source_raw_event_json->'tags') <> 'array' then
    raise exception 'verified Buzz event tags must be an array';
  end if;
  ...
  if source_event_id !~ '^[0-9a-f]{64}$'
    ...
    or source_event_kind < 0
    or source_event_created_at < 0
    or source_raw_event_json->>'sig' !~ '^[0-9a-f]{128}$' then
  ...
  insert into public.buzz_event_provenance (
    ...
    source_actor_pubkey, source_event_kind, source_event_created_at,
    source_event_id, true, source_raw_event_json
  ) on conflict (workspace_id, buzz_event_id) do nothing
  returning id into stored_provenance_id;

  if stored_provenance_id is null then
    return 'duplicate';
  end if;

  select signer_pubkey, channel_id
  into proposal_proposer_pubkey, proposal_channel_id
  from public.buzz_event_provenance
  where workspace_id = target_workspace_id
    and buzz_event_id = source_proposal_event_id
    and signature_verified;

  if proposal_proposer_pubkey is null
  ...
  if source_event_kind = 7 and source_raw_event_json->>'content' in ('+', '✅', '👍') then
    source_decision := 'approved';
  elsif source_event_kind = 7 and source_raw_event_json->>'content' in ('-', '❌', '👎') then
  ...
  elsif source_decision = 'rejected' then
...
revoke all on function public.record_buzz_event_provenance(uuid, uuid, text, text, text, text, text, integer, bigint, text, boolean, jsonb) from public, anon, authenticated, service_role;
revoke all on function public.apply_buzz_approval_transition(uuid, uuid, text, text, text, timestamptz, timestamptz, jsonb) from public, anon, authenticated, service_role;
revoke all on function public.apply_verified_buzz_approval(uuid, uuid, timestamptz, timestamptz, text, jsonb) from public, anon, authenticated;
grant execute on function public.apply_verified_buzz_approval(uuid, uuid, timestamptz, timestamptz, text, jsonb) to service_role;
