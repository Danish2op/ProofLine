...
begin
  insert into public.audit_events (
    ...
    'action_passport',
    new.id,
    case when tg_op = 'INSERT' then null else pg_catalog.encode(public.digest(pg_catalog.to_jsonb(old)::text, 'sha256'), 'hex') end,
    pg_catalog.encode(public.digest(pg_catalog.to_jsonb(new)::text, 'sha256'), 'hex'),
