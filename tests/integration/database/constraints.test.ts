...
describe('Proofline database constraints', () => {
  ...
  it('compares the complete rejection audit identity before accepting an exact replay', () => {
    const hardening = migration(
      '0017_task_8_rejection_audit_identity_hardening.sql',
    );

    expect(hardening).toContain(
      "existing_audit_aggregate_type is distinct from 'action_passport'",
    );
    expect(hardening).toContain(
      'existing_audit_metadata is distinct from audit_metadata',
    );
    expect(hardening).toContain(
      'existing_audit_actor_type is distinct from audit_actor_type',
    );
    expect(hardening).toContain(
      'existing_audit_actor_id is distinct from source_actor_id',
    );
    expect(hardening).toContain(
      'existing_audit_correlation_id is distinct from source_correlation_id',
    );
    expect(hardening).toContain(
      'existing_audit_causation_id is distinct from source_causation_id',
    );
    expect(hardening).toContain('existing_audit_before_hash is not null');
    expect(hardening).toContain('existing_audit_after_hash is not null');
    expect(hardening).toContain(
      "raise exception 'lifecycle rejection audit collision'",
    );
    expect(hardening).not.toContain('existing_audit_metadata->>');
  });
